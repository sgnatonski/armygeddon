...
module.exports = (config) => {
    const { mesh, spacing, mountainSpacing } = config;
    const filename = `build/points-${spacing}.data`;

    /* First generate the mountain points */
    let mountainPoints = new Poisson({ shape: [1000, 1000], minDistance: mountainSpacing }, makeRandFloat(mesh.seed)).fill();

    /* Generate the rest of the mesh points with the mountain points as constraints */
    let generator = new Poisson({ shape: [1000, 1000], minDistance: spacing }, makeRandFloat(mesh.seed));
